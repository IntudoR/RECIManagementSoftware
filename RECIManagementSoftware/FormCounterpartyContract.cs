...
{
    public partial class FormCounterpartyContract : Form
    {
        private static string _computerName = System.Environment.MachineName;

        public FormCounterpartyContract()
        {
            InitializeComponent();
        ...
        {
            var saveOrderId = textBoxContractOrderID.Text;

            if (saveOrderId == String.Empty)
            {
        ...
        private void AccountGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try 
            {
                if (ContractGridView.Rows[e.RowIndex].Cells[e.ColumnIndex].Value != null)
                {
                    ...
                    textBoxContractOrderID.Text = ContractGridView.SelectedRows[0].Cells["idOrder"].Value.ToString();
                    textBoxContractMonthlyPayment.Text = ContractGridView.SelectedRows[0].Cells["MonthlyPayment"].Value.ToString();
                }
